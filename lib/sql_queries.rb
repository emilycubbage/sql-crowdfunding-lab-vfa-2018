...
"SELECT projects.title, SUM(pledges.amount)-projects.funding_goal FROM pledges INNER JOIN projects ON  pledges.project_id = projects.id WHERE (pledges.amount - projects.funding_goal) >= -10 GROUP BY pledges.project_id;"
end

def selects_user_names_and_amounts_of_all_pledges_grouped_by_name_then_orders_them_by_the_amount_and_users_name
"SELECT users.name, SUM(pledges.amount) FROM users INNER JOIN pledges ON users.id = pledges.user_id GROUP BY users.name ORDER BY SUM(pledges.amount) ASC, users.name ASC;"
end
